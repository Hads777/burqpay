...
const BusBooking = () => {
  ...
  const getIbftData = async () => {
    try {
      ...
      const response = await getBusBooking(page);
      if (response) {
  ...
  const mappedData =
    ...
    const exportToExcel = () => {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "BusBooking");
      XLSX.writeFile(workbook, "BusBooking.xlsx");
    };
