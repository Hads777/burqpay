...
const MobileBundle = () => {
  ...
  const getIbftData = async () => {
    try {
      ...
      const response = await getMobileBundle(page);
      if (response) {
  ...
  const mappedData =
    ...
    const exportToExcel = () => {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "MobileBundle");
      XLSX.writeFile(workbook, "MobileBundle.xlsx");
    };
