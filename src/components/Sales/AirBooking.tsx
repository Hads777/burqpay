...
const AirBooking = () => {
  ...
  const getIbftData = async () => {
    try {
      ...
      const response = await getAirBooking(page);
      if (response) {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "AirBooking");
    XLSX.writeFile(workbook, "AirBooking.xlsx");
  };
