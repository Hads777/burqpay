...
import { setPackageDetails } from "../../redux/apis/apisSlice";
...
const OrderPackages = () => {
  ...
  const getPackages = async () => {
    try {
      ...
      const response = await getOrderPackages();
      if (response) {
  ...
  const menu = (row: any) => (
    <Menu>
      <Menu.Item
        ...
        // icon={< details/>}
        onClick={() => {
         navigate("Orders/Packages/Details");
          dispatch(setPackageDetails({ packageDetails: row?.details }));
        }}
      >
        Details
      </Menu.Item>
