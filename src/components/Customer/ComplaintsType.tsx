...
const ComplaintsType = () => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      setIsLoading(true);
      await toast.promise(
        deleteComplaintType(rowData?.id), // API call
        {
  ...
  const getComplaintsList = async () => {
    try {
      ...
      const response = await complaintTypeList(page, pageSize);
      if (response) {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      if (selectedItem === "edit") {
        ...
        await toast.promise(
          editComplaintType(updateBody), // Pass the body with ID included
          {
        ...
        await toast.promise(
          createComplaintType(formValues), // API call
          {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Complainttype");
    XLSX.writeFile(workbook, "complainttype.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    const tableColumn = ["ID", "Name", "Status"];
