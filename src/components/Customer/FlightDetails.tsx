...
const FlightDetails = () => {
  const [selectTab, setSelectedTab] = useState("fDetails");
  ...
  const customerDetails = [
    { label: "Order No", value: flightDetails?.order_number || "-" },
    { label: "Customer Name", value: flightDetails?.customer_name || "-" },
    { label: "Email", value: flightDetails?.email || "-" },
    { label: "Mobile No", value: flightDetails?.mobile_no || "-" },
    {
      label: "Order Status",
      value: (
      ...
      value:
        `${
          flightDetails?.data?.flight_details?.inbound
            ? flightDetails?.data?.flight_details?.inbound?.flightStartAirline
            : flightDetails?.data?.flight_details?.outbound
                ?.flightStartAirline || "----"
        }-${
          flightDetails?.data?.flight_details?.inbound
            ? flightDetails?.data?.flight_details?.inbound?.flightEndAirline
            : flightDetails?.data?.flight_details?.outbound?.flightEndAirline ||
              "----"
        }` || "----",
    },
  ];
  ...
  const additionalDetails = [
    { label: "PNR", value: flightDetails?.data?.booking_details?.pnr || "-" },
    {
      label: "Flight Type",
      value: flightDetails?.data?.flight_details?.flightDiresction || "-",
    },
    {
      label: "Air Line",
      value: flightDetails?.data?.flight_details?.airLineName || "-",
    },
    // { label: "Passengers", value:flightDetails?.data?.booking_details?.passengers?.length||"-" },
    {
      label: "Base Fare",
      value:
        `${
          flightDetails?.data?.flight_details?.flightsPricingSets[0]
            ?.priceDetail?.totalFare?.currency || "-"
        } ${
          flightDetails?.data?.flight_details?.flightsPricingSets[0]
            ?.priceDetail?.totalFare?.baseFareAmount || "-"
        }` || "-",
    },
    {
      label: "Tax",
      value:
        `${
          ...
          flightDetails?.data?.flight_details?.flightsPricingSets[0]
            ?.priceDetail?.totalFare?.totalTaxAmount || "-"
        }` || "-",
    },
    {
      label: "Amount",
      value:
        `${
          ...
          flightDetails?.data?.flight_details?.flightsPricingSets[0]
            ?.priceDetail?.totalFare?.totalPrice || "-"
        }` || "-",
    },
  ];
  const tapOptions = [
    {
      ...
      key: "fDetails",
      folder: <FlightDetailsTable />,
    },
    {
      title: "Passenger Details",
      key: "bDetails",
      folder: <BookingDetailsTable />,
    },
  ];
