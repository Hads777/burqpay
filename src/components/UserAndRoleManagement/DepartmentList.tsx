...
const DepartmentList = () => {
  ...
  const getAllUsers = async () => {
    ...
    try {
      const res = await getAllDepartments(page, pageSize);
      if (res?.data?.success) {
  ...
  const menu = (row: any) => (
    <Menu>
       {hasAccess("department_module","edit_department")&&
      <Menu.Item
        key="edit"
        icon={<EditOutlined />}
...
{hasAccess("department_module","delete_department")&&
      <Menu.Item
        key="delete"
        icon={<DeleteOutlined />}
        onClick={() => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      await toast.promise(
        deleteDepartent(rowData?.id, Body), // API call
        {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      if (selectedItem === "edit") {
        ...
        await toast.promise(updateDepartent(rowData?.id,updateBody), {
        ...
        await toast.promise(createDepartment(formValues), {
          ...
          success: (res) => {
            if (res.data.success) {
              ...
              getAllUsers();
              setOpenModal(false);
              return "DepartmentDepartment added successfully";
            }
            throw new Error(res.data.message || "Failed to add Department");
          },
          error: (err) => err?.message || "Failed to add Department",
        });
      }
    } catch (error) {
      console.error("Error:", error);
  ...
  return (
    <>
      <div className="service">
        ...
        <div className="d-flex justify-content-end col-12">
          ...
          <div className="d-flex gap-2 w-100">
            ...
            {hasAccess("department_module","create_department")&&
            <button
              className="theme-btn"
              onClick={() => {
                setOpenModal(true);
      ...
      <Modal
        ...
        title={selectedItem === "edit" ? "Edit Department" : "Add New Department"}
        visible={openModal}
