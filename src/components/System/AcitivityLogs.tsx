...
const ActivityLogs = () => {
  ...
  const menu = (row: any) => (
    <Menu>
      {hasAccess("customer_module", "update_customers") && (
        <Menu.Item
          key="edit"
          icon={<EditOutlined />}
  ...
  const Activity_Loans_Header = [
    ...
    {
      name: "Initiated On",
      cell: (row: { created_at: any }) => row.created_at,
      sortable: true,
    },
    // {
    //   name: "Activity Name",
  ...
  const getList = async () => {
    try {
      ...
      const response = await getActivitLog(page, pageSize);
      if (response) {
  ...
  const mappedData =
    ...
    data?.map((item: any, index: any) => {
      return {
        ...
        // accountType:
        //   item?.user_type_name == "saudi"
        //     ? "Citizen"
        //     : item?.user_type_name == "non-saudi"
        //     ? "Resident"
        //     : "Visitor",
